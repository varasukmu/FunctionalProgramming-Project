...
def executePipeline(mode: String): Either[String, ProcessData.AnalysisResult] =
	for {
		...
		dataWithCodes =
			if mode == "parallel" 
				then transformDataParallel(data, provinceMap, districtMap)
			else 
				transformData(data, provinceMap, districtMap)
...
def transformData(data: List[AccidentRecord], pMap: Map[String, String], dMap: Map[String, String]): Seq[AccidentRecordWithCode] =
	data.map(rec => buildRecord(rec, pMap, dMap))

def transformDataParallel(data: List[AccidentRecord], pMap: Map[String, String], dMap: Map[String, String]): Seq[AccidentRecordWithCode] =
